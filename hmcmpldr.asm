...
;
;	THE VERSION BELOW HAS BEEN MODIFIED 
;	TO LOAD THE 8000H MONITOR BIN FILE INTO RAM MEMORY AT
;		ADDRESS 8000H
;
...
;	THIS MAY BE DUE TO MY INTERFACE BETWEEN MY LAPTOP AND THE
;	 MEMBERSHIP CARD OR MY TERMINAL PROGRAM.
;
;	INVERTED LOGIC - /EF3 when idle is at VCC level
;	OPCODE LISTING OF THIS PROGRAM, (67 BYTES LONG)
;	0000	90 B4 A4 A5 F8 9C B5 E4 72 55 15 84 3A 08 F8 9C
;	0010	B0 3E 11 36 13 F8 80 B2 FE A2 E2 C4 C4 FC 01 3E
;	0020	1D B3 36 22 F8 FF 52 93 3E 28 F6 38 93 FF 01 3A
;	0030	2D 36 34 38 F6 02 76 52 C3 9C 2C 64 93 FF 01 3A
;	0040	3D 30 22
;
...
;	0030	2D 3E 34 38 F6 02 76 52 C3 9C 2C 64 93 FF 01 3A
;	0040	3D 30 22
;
;	
...
;	NOW SEND THE 8000H.BIN FILE FROM YOUR TERMINAL PROGRAM TO THE SERIAL PORT.
;	NOTE - 	THE LED'S ON THE FRONT WILL DISPLAY EACH BYTE THAT HAS BEEN TRANSFERRED.
;		WHEN THE 32KMON.BIN HAS BEEN COMPLETELY LOADED INTO THE MEMBERSHIP CARD
;		THE LEDS ON THE FRON PANEL SHOULD DISPLAY A HEX "0F" (00001111).
;	AFTER THE TRANSFER IS COMPLETE, NOW LOADED IN A JUMP TO ADDRESS 8000H (C0 80 00)
;	NOW RESET AND RESTART YOUR MEMBERSHIP CARD
...
;
;	Transfer Memory address 0000h page (256 bytes) from - to Memory address 9C00hex page (256 bytes)
;
;
...
;	Modified the software to transfer itself from address 0000h to address 9C00h so you can use it
;	to transfer the 8000h monitor bin file into the correct address (8000h) in order to run it from 
;	RAM memory
;
 
TPAGE	EQU	9CH	;MEMORY PAGE THAT THIS PROGRAM IS TRANSFERRED INTO
...
Page	EQU	80H	;example, load at 0000H
...
LoopB	B3 LoopB	;   loop while EF3 true (EF3 pin low)
	...
	PHI 2		;2    high byte = Page 80H
	SHL		;3    clear low byte 	
	PLO 2		;4    low byte = 00 (and sets D=0)
